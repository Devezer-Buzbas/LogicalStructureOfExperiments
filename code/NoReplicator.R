...
baseDir <- "/home/erkan/Desktop/lab/Reproducibility/MarkovChain" 
scriptDir <- paste0(baseDir, "/NoReplicator/code") # code directory
outputDir <- paste0(baseDir, "/NoReplicator/data") # data directory
#############
## FUNCTIONS
#############
...
sigma <- 0.5 #(1:4) error variance to model expectation
...
for ( l in 1:L){
  ...
  # precision.
  name <- paste('/home/erkan/lab/Reproducibility/MarkovChain/NoReplicator/data/ModelComparisonProbabilitiesSigma08AIC',l,'.RData', sep="")
  load(name)

  # SC
  Paic[[l]] <- ProbMC
  name <- paste('/home/erkan/lab/Reproducibility/MarkovChain/NoReplicator/data/ModelComparisonProbabilitiesSigma08Schwarz',l,'.RData', sep="")
  load(name)
  Psc[[l]] <- ProbMC
}
#------------------------------------------------------------
...
#
MeanProbStayInModelAllSC <- rep(0,L) # All scientists in equal proporitons, using SC
...
# Heatmap
a = 25
TauAllAIC[TauAllAIC>a]=a
TauTessAIC[TauTessAIC>a]=a
TauBoAIC[TauBoAIC>a]=a
TauMaveAIC[TauMaveAIC>a]=a
#
TauAllSC[TauAllSC>a]=a
TauTessSC[TauTessSC>a]=a
TauBoSC[TauBoSC>a]=a
TauMaveSC[TauMaveSC>a]=a

maxcol <- max(max(TauAllAIC),max(TauTessAIC),max(TauBoAIC),max(TauMaveAIC),
             max(TauAllSC),max(TauTessSC),max(TauBoSC),max(TauMaveSC))
mincol <- min(min(TauAllAIC),min(TauTessAIC),min(TauBoAIC),min(TauMaveAIC),
             min(TauAllSC),min(TauTessSC),min(TauBoSC),min(TauMaveSC)) 
...
PlotMFTPTessAIC <- levelplot(t(MFPTTessAIC.dat), 
          ...
          col.regions = heat.colors(a)[length(heat.colors(a)):b],
          main = "Tess")
...
grid.arrange(PlotMFTPTessAIC,PlotMFTPMaveAIC,PlotMFTPBoAIC,PlotMFTPAllAIC, 
             PlotMFTPTessSC, PlotMFTPMaveSC, PlotMFTPBoSC, PlotMFTPAllSC,
             ncol=4, nrow=2)
...
dfbestthree <- data.frame(cbind(AICTess[,4:6],AICMave[,4:6],AICBo[,4:6],AICAll[,4:6],
...
colnames(dfbestthree) <- c(rep("P(AICTess)",3),rep("P(AICMave)",3),rep("P(AICBo)",3),
                           rep("P(AICAll)",3),rep("P(SCTess)",3),rep("P(SCMave)",3),
                           rep("P(SCBo)",3),rep("P(SCAll)",3),rep("M(AICTess)",3),
                           rep("M(AICMave)",3),rep("M(AICBo)",3),rep("M(AICAll)",3),
                           rep("M(SCTess)",3),rep("M(SCMave)",3),rep("M(SCBo)",3),
                           rep("M(SCAll)",3))
#
